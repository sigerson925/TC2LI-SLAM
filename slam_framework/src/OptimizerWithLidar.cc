...
LIDAR::LidarParam* OptimizerWithLidar::mLidarParam = new LIDAR::LidarParam();

void OptimizerWithLidar::LocalLVBundleAdjustment(KeyFrame *pKF, bool* pbStopFlag, Map* pMap, int& num_fixedKF, int& num_OptKF, int& num_MPs, int& num_edges)
{
    ...
    // LiDAR Co-vis Residual
    vector<KeyFrame*> vOptKeyFrames;
    for(list<KeyFrame*>::iterator lit=lLocalKeyFrames.begin(), lend=lLocalKeyFrames.end(); lit!=lend; lit++)
    {
        KeyFrame* pKFi = *lit;
        if(pKFi->mLidarProps->GetSurfacePcl()->size() > 0)
            vOptKeyFrames.push_back(pKFi);
    }

    if(vOptKeyFrames.size() > 2)
    {
        LidarCovisRes* pLio;
        if(pbStopFlag)
            pLio = new LidarCovisRes(mLidarParam->mTcl, mLidarParam->mTbl, 0.0, pbStopFlag);
        else
            pLio = new LidarCovisRes(mLidarParam->mTcl, mLidarParam->mTbl);
        EdgeLidarSE3<g2o::VertexSE3Expmap>* eBalm = new EdgeLidarSE3<g2o::VertexSE3Expmap>(pLio);

        pLio->win_size_ = vOptKeyFrames.size();
        if(pLio->win_size_ > 6) pLio->win_size_ = 6;
        eBalm->resize(pLio->win_size_);
        for(int i=0 ;i < pLio->win_size_;++i)
        {
            KeyFrame* pKFi = vOptKeyFrames[i];
            pLio->AddFromKeyFrame(pKFi);
            g2o::HyperGraph::Vertex* VP =  optimizer.vertex(pKFi->mnId);
            eBalm->setVertex(i, VP);
        }

        pLio->BuildVoxHess();
        double h = mLidarParam->mWeightLocalBA;
        eBalm->setInformation(Eigen::Matrix<double, 1, 1, 0, 1, 1>::Identity()*h);
        optimizer.addEdge(eBalm);
...
void OptimizerWithLidar::LocalLVIBA(KeyFrame *pKF, bool *pbStopFlag, Map *pMap, int& num_fixedKF, int& num_OptKF, int& num_MPs, int& num_edges, bool bLarge, bool bRecInit)
{
    Map* pCurrentMap = pKF->GetMap();
    ...
    LidarCovisRes* pLio = new LidarCovisRes(mLidarParam->mTcl, mLidarParam->mTbl);
    
    vector<KeyFrame*> vLiDAROptKeyFrames;
    for(vector<KeyFrame*>::iterator lit=vpOptimizableKFs.begin(), lend=vpOptimizableKFs.end(); lit!=lend; lit++)
    {
        KeyFrame* pKFi = *lit;
        if(pKFi->mLidarProps->GetSurfacePcl()->size() > 0)
            vLiDAROptKeyFrames.push_back(pKFi);
    }

    //lidar co-vision edge
    EdgeLidar* eLidar = new EdgeLidar(pLio);
    int N1 = N;
    if(N1 > 5)
    {
        if(N1>6) N1 = 6;
        ...
        for(int i=0;i<N1;i++)
        {
            ...
            pLio->AddFromKeyFrame(pKFi);
            g2o::HyperGraph::Vertex* VP =  optimizer.vertex(pKFi->mnId);
            eLidar->setVertex(i, VP);
        }
        pLio->BuildVoxHess();
        double h = mLidarParam->mWeightLocalBA;

        eLidar->setInformation(Eigen::Matrix<double, 1, 1, 0, 1, 1>::Identity() * h);
        optimizer.addEdge(eLidar);
    }
