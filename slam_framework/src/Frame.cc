     ...
     mpCamera(pCamera) ,mpCamera2(nullptr), mbHasPose(false), mbHasVelocity(false)
{
    ...
    mLidarProps = new LIDAR::LidarProperties();
...
#endif
    TicToc threadORBextract(1);
    thread threadLeft(&Frame::ExtractORB,this,0,imLeft,0,0);
    thread threadRight(&Frame::ExtractORB,this,1,imRight,0,0);
    threadLeft.join();
    threadRight.join();
    // threadORBextract.toc("[Frame]: Extract Whole");
#ifdef REGISTER_TIMES
    ...
    if(mvKeys.empty())
        return;
    threadORBextract.tic();
    UndistortKeyPoints();
    // threadORBextract.toc("[Frame]: UndistortKeyPoints");
#ifdef REGISTER_TIMES
    std::chrono::steady_clock::time_point time_StartStereoMatches = std::chrono::steady_clock::now();
#endif
    threadORBextract.tic();
    ComputeStereoMatches();
    // threadORBextract.toc("[Frame]: ComputeStereoMatches");
#ifdef REGISTER_TIMES
...
void Frame::ExtractORB(int flag, const cv::Mat &im, const int x0, const int x1)
{
    ...
    // threadORBextract.toc("[Frame]: ExtractORB");
}
         ...
         mbHasPose(false), mbHasVelocity(false)
{
    ...
    // threadORBextract.toc("[Frame]: Compute Matches");
#ifdef REGISTER_TIMES
    std::chrono::steady_clock::time_point time_EndStereoMatches = std::chrono::steady_clock::now();
