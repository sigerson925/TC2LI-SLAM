...
{
    mCurrInputCloud.reset(new pcl::PointCloud<PointXYZINormal>());
    mLastInputCloud.reset(new pcl::PointCloud<PointXYZINormal>());
    // Load camera parameters from settings file
    if(settings){
        newParameterLoader(settings);
        cv::FileStorage fSettings(strSettingPath, cv::FileStorage::READ);
        cv::FileNode node = fSettings["LiDAR.enabled"];
        if(!node.empty()&& node.isInt())
        {   
            bool b_parse_lidar;
            mLidarEnabled = node.operator int();
            if(mLidarEnabled)
            {
                b_parse_lidar = ParseLiDARParamFile(fSettings);
                if(!b_parse_lidar)
                {
                    std::cout << "*Error with the LiDAR parameters in the config file*" << std::endl;
                }
            }
        ...
        if(sensor==System::IMU_STEREO_LIDAR)
        {
        ...
        if(!b_parse_cam || !b_parse_orb || !b_parse_imu||!b_parse_lidar)
        {
...
void Tracking::newParameterLoader(Settings *settings) {
    ...
    if((mSensor==System::STEREO_LIDAR || mSensor == System::IMU_STEREO_LIDAR) &&
...
bool Tracking::ParseLiDARParamFile(cv::FileStorage &fSettings)
{
    bool b_miss_params = false;
    cv::FileNode node = fSettings["Tcl"];
    cv::Mat cvTcl;
    if(!node.empty())
    {
        cvTcl = node.mat();
        if(cvTcl.rows != 4 || cvTcl.cols != 4)
        {
            std::cerr << "*Tcl matrix have to be a 4x4 transformation matrix*" << std::endl;
            b_miss_params = true;
        }
    }
    else
    {
        std::cerr << "*Tcl matrix doesn't exist*" << std::endl;
        b_miss_params = true;
    }
    cout << endl;
    cout << "LiDAR to Left camera Transform (Tcl): " << endl << cvTcl << endl;
    Eigen::Matrix<float,4,4,Eigen::RowMajor> eigTcl(cvTcl.ptr<float>(0));
    Sophus::SE3f Tcl(eigTcl);

    double wLBA;
    node = fSettings["LiDAR.wLBA"];
    if(!node.empty() && node.isReal())
    {
        wLBA = node.real();
    }
    ...
    {
        std::cerr << "*LiDAR.wLBA parameter doesn't exist or is not a real number*" << std::endl;
        b_miss_params = true;
    }

    cout << "LiDAR wLBA: " << wLBA << endl;

    if(!mpImuCalib)
    {
        Sophus::SE3f Tbc;
        mpLidarParam = new LIDAR::LidarParam(Tcl, Tbc, wLBA);
        OptimizerWithLidar::SetLidarParam(mpLidarParam);
        LIDAR::LidarFrontEndTools::SetLidarParam(mpLidarParam);
    }
    else
    {
        mpLidarParam = new LIDAR::LidarParam(Tcl, mpImuCalib->mTbc, wLBA);
        OptimizerWithLidar::SetLidarParam(mpLidarParam);
        LIDAR::LidarFrontEndTools::SetLidarParam(mpLidarParam);
    }
...
void Tracking::SetLocalMapper(LocalMapping *pLocalMapper)
{
    ...
    if(mLidarEnabled)
    {
        mpLocalMapper->mLidarEnabled = 1;
        mpLocalMapper->mLidarParam = new LIDAR::LidarParam(*mpLidarParam);
    }
}
...
void Tracking::BuildLidarFeat4KeyFrame()
{
    if (mCurrentFrame.mnId == mCurrentLidarId) // No further operation required
    {
        return;
    }
    // Virables for interpolation
    int lidarId =  mCurrentLidarId;
    if(mCurrentFrame.mnId < mCurrentLidarId)
        lidarId = mLastLidarId;
    if (lidarId < 0)
        return;
    int forwarNum;
    if(lidarId ==0 )
        ...
        forwarNum = lidarId - mDitchedNum;

    // Get frame pose
    std::list<Sophus::SE3f>::iterator it_pose = std::next(mlRelativeFramePoses.begin(), forwarNum);
    std::list<TC2LI_SLAM::KeyFrame *>::iterator it_refkf = std::next(mlpReferences.begin(), forwarNum);
    TC2LI_SLAM::KeyFrame *refKeyFrame = *it_refkf;
    std::list<double>::iterator it_time = std::next(mlFrameTimes.begin(), forwarNum);
    double timeLidar = *it_time;

    if (refKeyFrame == nullptr)
    {
        std::cout << "ref Keyframe is null !!!" << std::endl;
        return;
    }

    Sophus::SE3f poseLidar_cw = (*it_pose) * refKeyFrame->GetPose();
    pcl::PointCloud<PointXYZINormal>::Ptr cloudSurf;
    if(mCurrentFrame.mnId > mCurrentLidarId)
        cloudSurf.reset(new pcl::PointCloud<PointXYZINormal>(*mCurrInputCloud));

    // Convert point cloud using relative pose
    Sophus::SE3f transformOffset = LIDAR::LidarFrontEndTools::mLidarParam->mTlc * mCurrentFrame.GetPose()* poseLidar_cw.inverse() * LIDAR::LidarFrontEndTools::mLidarParam->mTcl;
    cloudSurf = LIDAR::LidarFrontEndTools::transformPointCloud(cloudSurf, transformOffset);
    mCurrentFrame.mLidarProps->SetPcl(cloudSurf);
}

Sophus::SE3f InterpolateSE3(const Sophus::SE3f& se3_1, const Sophus::SE3f& se3_2, float t)
{
    Eigen::Quaternionf q1(se3_1.rotationMatrix());
    Eigen::Quaternionf q2(se3_2.rotationMatrix());
    Eigen::Quaternionf q_interp = q1.slerp(t, q2); // 旋转矩阵插值

    Eigen::Vector3f p1 = se3_1.translation();
    Eigen::Vector3f p2 = se3_2.translation();
    Eigen::Vector3f p_interp = p1 + t * (p2 - p1); // 平移向量插值

    return Sophus::SE3f(q_interp, p_interp);
}

void Tracking::SyncWithLidar()
{
    unique_lock<mutex> lock(finishMutex);
    if(mLastFrame.mnId < 0) return;

    int waitCount = 0;
    if(mSensor == TC2LI_SLAM::System::IMU_STEREO_LIDAR) // wait for lidar front end to finish
        while(!lidarFinished)
        {
            usleep(1000);
            if(++waitCount > 50)
    ...
    // first check if the ids are matched
    int tempLiDARId = GetCurrCameraId();

    if(tempLiDARId == -1)
        return;
    if(mLastLidarId == tempLiDARId && tempLiDARId < mCurrentFrame.mnId)
        return;
    if(mCurrentFrame.mnId != tempLiDARId && mLastFrame.mnId != tempLiDARId)
        return;

    mLastLidarId = mCurrentLidarId;
    mCurrentLidarId = tempLiDARId;
    mCurrCloudTime = GetCurrTimeStamp();
    currentCloud = GetCurrFeatPoints();
    pcl::PointCloud<PointXYZINormal>::Ptr cloudSurf(new pcl::PointCloud<PointXYZINormal>());
    cloudSurf->clear();
    cloudSurf->reserve(currentCloud->size());
    // check if there's a paired scan
    if ( mCurrentFrame.mnId == mCurrentLidarId )
    {
        if(mSensor == System::STEREO_LIDAR)
            UpdateMap(mCurrentFrame);
        // Interpolate Pose
        Sophus::SE3f cloudTwc = InterpolateSE3(mLastFrame.GetPose().inverse(), mCurrentFrame.GetPose().inverse(),
        ( mCurrCloudTime - mLastFrame.mTimeStamp )/( mCurrentFrame.mTimeStamp - mLastFrame.mTimeStamp )
        );
        // Convert Point Cloud LiDAR -> Camera
        Sophus::SE3f transformOffset = LIDAR::LidarFrontEndTools::mLidarParam->mTlc * mCurrentFrame.GetPose() * cloudTwc * LIDAR::LidarFrontEndTools::mLidarParam->mTcl;
        cloudSurf = LIDAR::LidarFrontEndTools::transformPointCloud(currentCloud, transformOffset);
        mCurrentFrame.mLidarProps->SetPcl(cloudSurf);
    }
    else if(mLastFrame.mnId == mCurrentLidarId){
        if(mSensor == System::STEREO_LIDAR)
            UpdateMap(mLastFrame);
        ...
        // Convert Point Cloud LiDAR -> Camera
        Sophus::SE3f transformOffset = LIDAR::LidarFrontEndTools::mLidarParam->mTlc * mLastFrame.GetPose() * cloudTwc * LIDAR::LidarFrontEndTools::mLidarParam->mTcl;
        cloudSurf = LIDAR::LidarFrontEndTools::transformPointCloud(currentCloud, transformOffset);
        mLastFrame.mLidarProps->SetPcl(cloudSurf);
    }
    mLastInputCloud->clear();
    mLastInputCloud.reset(new pcl::PointCloud<PointXYZINormal>(*mCurrInputCloud));
    mCurrInputCloud.reset(new pcl::PointCloud<PointXYZINormal>(*cloudSurf));
}

Sophus::SE3f
Tracking::GrabImageStereoLidar(const cv::Mat &imRectLeft, const cv::Mat &imRectRight, const double &timestamp,
                               string filename)
{

    // Set current input cloud and cloud time
    double timeDiff = mCurrCloudTime - mCurrentFrame.mTimeStamp;
    double ratio = timestamp == mCurrentFrame.mTimeStamp? 1:(timeDiff)/(timestamp - mCurrentFrame.mTimeStamp);

    // Set current grayscale image and convert to grayscale if necessary
    mImGray = imRectLeft;
    ...
    // Update Lidar Pose
    if(mSensor == System::STEREO_LIDAR)
        UpdateLidarPose(mCurrentFrame, mVelocity, ratio);

    pcl::PointCloud<PointXYZINormal>::Ptr pCloud(new pcl::PointCloud<PointXYZINormal>());

    if (mSensor == System::STEREO_LIDAR && !mpCamera2)
        mCurrentFrame = Frame(mImGray, imGrayRight, timestamp, mpORBextractorLeft, mpORBextractorRight, mpORBVocabulary, mK, mDistCoef, mbf, mThDepth, mpCamera, &mLastFrame);
    else if(mSensor == System::STEREO_LIDAR && mpCamera2)
        mCurrentFrame = Frame(mImGray,imGrayRight,timestamp,mpORBextractorLeft,mpORBextractorRight,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth,mpCamera,mpCamera2,mTlr);
    else if(mSensor == System::IMU_STEREO_LIDAR && !mpCamera2)
    ...
    mCurrentFrame.mLidarProps->SetCloudTime(mCurrCloudTime);
...
void Tracking::PreintegrateIMU()
{
    ...
    if(n==0){
        ...
        cout << mvImuFromLastFrame[0].t << endl;
        return;
    }
...
bool Tracking::PredictStateIMU()
{
    ...
    else if(!mbMapUpdated)
    {
        ...
        // cout << setprecision(3) << "[Tracking]: Delta Velocity: " << mCurrentFrame.mpImuPreintegratedFrame->GetDeltaVelocity(mLastFrame.mImuBias) << endl;
        mCurrentFrame.SetImuPoseVelocity(Rwb2,twb2,Vwb2);
...
void Tracking::Track()
{
    ...
    if ((mSensor == System::IMU_STEREO_LIDAR) && mpLastKeyFrame)
    ...
    if ((mSensor == System::IMU_STEREO_LIDAR) && !mbCreatedMap)
    {
#ifdef REGISTER_TIMES
    ...
    else
    {
        ...
        if(!mbOnlyTracking)
        {
            ...
            if(mState==OK)
            {
                ...
                else
                {
                    ...
                    if(!bOK)
                    {
                        // std::cout<< "TRACK: Motion model failed, use reference KF " << std::endl;
                        bOK = TrackReferenceKeyFrame();
                    }
                }
                ...
                if (!bOK)
                {
                    if ( mCurrentFrame.mnId<=(mnLastRelocFrameId+mnFramesToResetIMU) &&
                         (mSensor==System::IMU_STEREO_LIDAR))
                    {
                        mState = LOST;
                    }
            ...
            else
            {
                ...
                if (mState == RECENTLY_LOST)
                {
                    ...
                    bOK = true;
                    if((mSensor == System::IMU_STEREO_LIDAR))
                    {
                        ...
                        if (mCurrentFrame.mTimeStamp-mTimeStampLost>time_recently_lost)
                        {
                            ...
                            cout << "Track Lost... IMU Reloc Failed..."<<endl;
                            bOK=false;
                        }
                    }
                    else
                    {
                        ...
                        if(mCurrentFrame.mTimeStamp-mTimeStampLost>3.0f && !bOK)
                        {
                            mState = LOST;
                            Verbose::PrintMess("Track Lost...", Verbose::VERBOSITY_NORMAL);
                            cout << "Track Lost... Reloc Failed..."<<endl;
                            bOK=false;
                        }
                    }
                }
        ...
        if((mCurrentFrame.mnId<(mnLastRelocFrameId+mnFramesToResetIMU)) && (mCurrentFrame.mnId > mnFramesToResetIMU) &&
           (mSensor == System::IMU_STEREO_LIDAR) && pCurrentMap->isImuInitialized())
        {
        ...
        if(mState==LOST)
        {
            ...
            if (mSensor == System::IMU_STEREO_LIDAR)
                if (!pCurrentMap->isImuInitialized())
                {
                    ...
                    cout<<"Track lost before IMU initialisation, reseting..."<<endl;
                    mpSystem->ResetActiveMap();
                    return;
                }
    ...
    if(mCurrentFrame.mnId >= mlpReferences.size())
        mDitchedNum = mCurrentFrame.mnId - mlpReferences.size() + 1;
...
{
    cout  << "\033[32mStereoInitialization\033[0m"<< endl ;
    ...
    void Tracking::CreateMapInAtlas()
{
    ...
    if((mSensor == System::IMU_STEREO_LIDAR) && mpImuPreintegratedFromLastKF)
    {
        delete mpImuPreintegratedFromLastKF;
        mpImuPreintegratedFromLastKF = new IMU::Preintegrated(IMU::Bias(),*mpImuCalib);
    }
...
bool Tracking::TrackWithMotionModel()
{
    ...
    // cout << "Track Motion: Update Last Frame " << endl;   
    UpdateLastFrame();

    if (mpAtlas->isImuInitialized() && (mCurrentFrame.mnId>mnLastRelocFrameId+mnFramesToResetIMU))
    {
        // Predict state with IMU if it is initialized and it doesnt need reset
        // cout << "Track Motion: predict IMU " << endl;
        PredictStateIMU();
        return true;
    }
    else
    ...
    int nmatches = matcher.SearchByProjection(mCurrentFrame,mLastFrame,th,0);

    // If few matches, uses a wider window search
    if(nmatches<20)
    {
        Verbose::PrintMess("Not enough matches, wider window search!!", Verbose::VERBOSITY_NORMAL);
        cout << "Track Motion: Not enough matches, wider window search!!" << endl;
        fill(mCurrentFrame.mvpMapPoints.begin(),mCurrentFrame.mvpMapPoints.end(),static_cast<MapPoint*>(NULL));

        nmatches = matcher.SearchByProjection(mCurrentFrame,mLastFrame,2*th,0);
        Verbose::PrintMess("Matches with wider search: " + to_string(nmatches), Verbose::VERBOSITY_NORMAL);
        cout << "Track Motion: Matches with wider search: " + to_string(nmatches) << endl;
    }

    if(nmatches<20)
    {
        Verbose::PrintMess("Not enough matches!!", Verbose::VERBOSITY_NORMAL);
        cout<<"Track Motion: Still Not enough matches!!"<<endl;
        if (mSensor == System::IMU_STEREO_LIDAR)
            return true;
        else
            return false;
...
bool Tracking::TrackLocalMap()
{
    ...
    for(int i=0; i<mCurrentFrame.N; i++)
    {
        if(mCurrentFrame.mvpMapPoints[i])
        {
            ...
            else if(mSensor==System::STEREO_LIDAR)
                mCurrentFrame.mvpMapPoints[i] = static_cast<MapPoint*>(NULL);
        }
    }
...
{
    if((mSensor == System::IMU_STEREO_LIDAR) && !mpAtlas->GetCurrentMap()->isImuInitialized())
    {
        if ((mSensor == System::IMU_STEREO_LIDAR) && (mCurrentFrame.mTimeStamp - mpLastKeyFrame->mTimeStamp) >= 0.25)
            return true;
        else
            return false;
    }
    ...
    if(nClosedPoints < thStereoClosedPoints && (mSensor==System::STEREO_LIDAR || mSensor==System::IMU_STEREO_LIDAR))
    {
    ...
    const bool c1c = mSensor!=System::IMU_STEREO_LIDAR && (mnMatchesInliers < nRefMatches * 0.25 || bNeedToInsertClose) ;
    ...
    bool c4 = false;
    if ((((mnMatchesInliers<75) && (mnMatchesInliers>15)) || mState==RECENTLY_LOST) && (0)) // MODIFICATION_2, originally ((((mnMatchesInliers<75) && (mnMatchesInliers>15)) || mState==RECENTLY_LOST) && ((mSensor == System::IMU_MONOCULAR)))
        c4=true;
    else
        c4=false;
...
void Tracking::CreateNewKeyFrame()
{
    ...
    // prepare planar feature
    if(mLidarEnabled)
        BuildLidarFeat4KeyFrame();
    KeyFrame* pKF = new KeyFrame(mCurrentFrame,mpAtlas->GetCurrentMap(),mpKeyFrameDB);
    static int count = 0;
    static int no_lidar_count = 0;

    ++count;
    
    if(mCurrentFrame.mLidarProps->GetSurfacePcl()->size() <= 0 ) ++no_lidar_count;
    else{
        pKF->mHasPointCloud = true;
    }
    ...
    if(1) // TODO check if incluide imu_stereo
    {
...
void Tracking::SearchLocalPoints()
{
    ...
    if(nToMatch>0)
    {
        ...
        else if(!mpAtlas->isImuInitialized() && (mSensor==System::IMU_STEREO_LIDAR))
        {
            th=10;
        }
...
void Tracking::UpdateLocalKeyFrames()
{
    ...
    if((mSensor == System::IMU_STEREO_LIDAR) && mvpLocalKeyFrames.size() < 80)
    {
...
void Tracking::UpdateFrameIMU(const float s, const IMU::Bias &b, KeyFrame* pCurrentKeyFrame)
{
    ...
    unsigned int index = mnFirstFrameId;
    list<TC2LI_SLAM::KeyFrame*>::iterator lRit = mlpReferences.begin();
