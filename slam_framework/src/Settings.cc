...
namespace TC2LI_SLAM {
    ...
    bNeedToUndistort_(false), bNeedToRectify_(false), bNeedToResize1_(false), bNeedToResize2_(false) {
        ...
        if(sensor_ == System::STEREO_LIDAR || sensor_ == System::IMU_STEREO_LIDAR){
        ...
        if(sensor_ == System::IMU_STEREO_LIDAR){
            readIMU(fSettings);
            cout << "\t-Loaded IMU calibration" << endl;
        }

        cv::FileStorage fsSettings(configFile.c_str(), cv::FileStorage::READ);
        cv::FileNode node = fsSettings["LiDAR.enabled"];
        if(!node.empty()&& node.isInt())
        {   
            bool lidarEnabled = node.operator int();
            if(lidarEnabled)
            {
    ...
    void Settings::readImageInfo(cv::FileStorage &fSettings) {
        ...
        if(found){
            ...
            if(!bNeedToRectify_){
                ...
                if((sensor_ == System::STEREO_LIDAR || sensor_ == System::IMU_STEREO_LIDAR) && cameraType_ != Rectified){
    ...
    void readLiDAR(cv::FileStorage& fSettings)
    {
    }
    ...
    ostream &operator<<(std::ostream& output, const Settings& settings){
        ...
        if(settings.sensor_ == System::STEREO_LIDAR || settings.sensor_ == System::IMU_STEREO_LIDAR){
        ...
        if(settings.sensor_ == System::IMU_STEREO_LIDAR) {
