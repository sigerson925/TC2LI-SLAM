...
KD_TREE<PointType_ikd>::KD_TREE(float delete_param, float balance_param, float box_length)
{
...
template <typename PointType_ikd>
KD_TREE<PointType_ikd>::~KD_TREE()
{
    stop_thread();
    Delete_Storage_Disabled = true;
    delete_tree_nodes(&Root_Node);
    PointVector_ikd().swap(PCL_Storage);
    Rebuild_Logger.clear();
}
...
template <typename PointType_ikd>
void KD_TREE<PointType_ikd>::InitializeKDTree(float delete_param, float balance_param, float box_length)
{
...
template <typename PointType_ikd>
void KD_TREE<PointType_ikd>::InitTreeNode(KD_TREE_NODE *root)
{
    root->point.x = 0.0f;
    root->point.y = 0.0f;
    root->point.z = 0.0f;
...
template <typename PointType_ikd>
int KD_TREE<PointType_ikd>::size()
{
    int s = 0;
...
template <typename PointType_ikd>
BoxPointType KD_TREE<PointType_ikd>::tree_range()
{
    BoxPointType range;
    if (Rebuild_Ptr == nullptr || *Rebuild_Ptr != Root_Node)
    {
...
template <typename PointType_ikd>
int KD_TREE<PointType_ikd>::validnum()
{
    int s = 0;
    if (Rebuild_Ptr == nullptr || *Rebuild_Ptr != Root_Node)
    {
...
template <typename PointType_ikd>
void KD_TREE<PointType_ikd>::root_alpha(float &alpha_bal, float &alpha_del)
{
    if (Rebuild_Ptr == nullptr || *Rebuild_Ptr != Root_Node)
    {
...
template <typename PointType_ikd>
void KD_TREE<PointType_ikd>::start_thread()
{
...
template <typename PointType_ikd>
void KD_TREE<PointType_ikd>::stop_thread()
{
...
template <typename PointType_ikd>
void *KD_TREE<PointType_ikd>::multi_thread_ptr(void *arg)
{
...
template <typename PointType_ikd>
void KD_TREE<PointType_ikd>::multi_thread_rebuild()
{
    ...
    while (!terminated)
    {
        ...
        if (Rebuild_Ptr != nullptr)
        {
            ...
            PointVector_ikd().swap(Rebuild_PCL_Storage);
            // Lock Search
...
template <typename PointType_ikd>
void KD_TREE<PointType_ikd>::run_operation(KD_TREE_NODE **root, Operation_Logger_Type operation)
{
    switch (operation.op)
    {
    case ADD_POINT:
...
template <typename PointType_ikd>
void KD_TREE<PointType_ikd>::Build(PointVector_ikd point_cloud)
{
    if (Root_Node != nullptr)
    {
        delete_tree_nodes(&Root_Node);
...
template <typename PointType_ikd>
void KD_TREE<PointType_ikd>::Nearest_Search(PointType_ikd point, int k_nearest, PointVector_ikd &Nearest_Points, vector<float> &Point_Distance, float max_dist)
{
    ...
    PointVector_ikd().swap(Nearest_Points);
    vector<float>().swap(Point_Distance);
...
template <typename PointType_ikd>
void KD_TREE<PointType_ikd>::Box_Search(const BoxPointType &Box_of_Point, PointVector_ikd &Storage)
{
    Storage.clear();
...
template <typename PointType_ikd>
void KD_TREE<PointType_ikd>::Radius_Search(PointType_ikd point, const float radius, PointVector_ikd &Storage)
{
    Storage.clear();
...
template <typename PointType_ikd>
int KD_TREE<PointType_ikd>::Add_Points(PointVector_ikd &PointToAdd, bool downsample_on)
{
    int NewPointSize = PointToAdd.size();
    int tree_size = size();
    BoxPointType Box_of_Point;
    PointType_ikd downsample_result, mid_point;
    bool downsample_switch = downsample_on && DOWNSAMPLE_SWITCH;
    float min_dist, tmp_dist;
    int tmp_counter = 0;

    double search_time = 0.0;TicToc searchTime;
    double add_time = 0.0;TicToc addTime;
    ...
    for (int i = 0; i < PointToAdd.size(); i++)
    {
        if (downsample_switch)
        {
            ...
            PointVector_ikd().swap(Downsample_Storage);
            searchTime.tic();
            Search_by_range(Root_Node, Box_of_Point, Downsample_Storage);
            search_time += searchTime.toc(" ");
...
template <typename PointType_ikd>
void KD_TREE<PointType_ikd>::Add_Point_Boxes(vector<BoxPointType> &BoxPoints)
{
...
template <typename PointType_ikd>
void KD_TREE<PointType_ikd>::Delete_Points(PointVector_ikd &PointToDel)
{
...
template <typename PointType_ikd>
int KD_TREE<PointType_ikd>::Delete_Point_Boxes(vector<BoxPointType> &BoxPoints)
{
    int tmp_counter = 0;
    for (int i = 0; i < BoxPoints.size(); i++)
    {
...
template <typename PointType_ikd>
void KD_TREE<PointType_ikd>::acquire_removed_points(PointVector_ikd &removed_points)
{
    pthread_mutex_lock(&points_deleted_rebuild_mutex_lock);
...
template <typename PointType_ikd>
void KD_TREE<PointType_ikd>::BuildTree(KD_TREE_NODE **root, int l, int r, PointVector_ikd &Storage)
{
    if (l > r)
        return;
...
template <typename PointType_ikd>
void KD_TREE<PointType_ikd>::Rebuild(KD_TREE_NODE **root)
{
...
template <typename PointType_ikd>
int KD_TREE<PointType_ikd>::Delete_by_range(KD_TREE_NODE **root, BoxPointType boxpoint, bool allow_rebuild, bool is_downsample)
{
...
template <typename PointType_ikd>
void KD_TREE<PointType_ikd>::Delete_by_point(KD_TREE_NODE **root, PointType_ikd point, bool allow_rebuild)
{
    if ((*root) == nullptr || (*root)->tree_deleted)
        return;
    (*root)->working_flag = true;
...
template <typename PointType_ikd>
void KD_TREE<PointType_ikd>::Add_by_range(KD_TREE_NODE **root, BoxPointType boxpoint, bool allow_rebuild)
{
    if ((*root) == nullptr)
        return;
    (*root)->working_flag = true;
...
template <typename PointType_ikd>
void KD_TREE<PointType_ikd>::Add_by_point(KD_TREE_NODE **root, PointType_ikd point, bool allow_rebuild, int father_axis)
{
    if (*root == nullptr)
    {
        *root = new KD_TREE_NODE;
...
template <typename PointType_ikd>
void KD_TREE<PointType_ikd>::Search(KD_TREE_NODE *root, int k_nearest, PointType_ikd point, MANUAL_HEAP &q, float max_dist)
{
    if (root == nullptr || root->tree_deleted)
        return;
...
template <typename PointType_ikd>
void KD_TREE<PointType_ikd>::Search_by_range(KD_TREE_NODE *root, BoxPointType boxpoint, PointVector_ikd &Storage)
{
    if (root == nullptr)
        return;
    ...
    if (boxpoint.vertex_min[0] <= root->node_range_x[0] && boxpoint.vertex_max[0] > root->node_range_x[1] && boxpoint.vertex_min[1] <= root->node_range_y[0] && boxpoint.vertex_max[1] > root->node_range_y[1] && boxpoint.vertex_min[2] <= root->node_range_z[0] && boxpoint.vertex_max[2] > root->node_range_z[1])
    {
        ...
        // searchTime.toc("[Search_by_range] flatten");
...
template <typename PointType_ikd>
void KD_TREE<PointType_ikd>::Search_by_range_non_recur(KD_TREE_NODE *root, BoxPointType boxpoint, PointVector_ikd &Storage)
{
    if (root == nullptr)
        return;
    Push_Down(root);
...
template <typename PointType_ikd>
void KD_TREE<PointType_ikd>::Search_by_radius(KD_TREE_NODE *root, PointType_ikd point, float radius, PointVector_ikd &Storage)
{
    if (root == nullptr)
        return;
    Push_Down(root);
    PointType_ikd range_center;
...
template <typename PointType_ikd>
bool KD_TREE<PointType_ikd>::Criterion_Check(KD_TREE_NODE *root)
{
...
template <typename PointType_ikd>
void KD_TREE<PointType_ikd>::Push_Down(KD_TREE_NODE *root)
{
    if (root == nullptr)
        return;
    Operation_Logger_Type operation;
...
template <typename PointType_ikd>
void KD_TREE<PointType_ikd>::Update(KD_TREE_NODE *root)
{
...
template <typename PointType_ikd>
void KD_TREE<PointType_ikd>::flatten(KD_TREE_NODE *root, PointVector_ikd &Storage, delete_point_storage_set storage_type)
{
    if (root == nullptr)
        return;
    Push_Down(root);
...
template <typename PointType_ikd>
void KD_TREE<PointType_ikd>::delete_tree_nodes(KD_TREE_NODE **root)
{
    if (*root == nullptr)
        return;
    Push_Down(*root);
...
template <typename PointType_ikd>
bool KD_TREE<PointType_ikd>::same_point(PointType_ikd a, PointType_ikd b)
{
    return (fabs(a.x - b.x) < EPSS && fabs(a.y - b.y) < EPSS && fabs(a.z - b.z) < EPSS);
}

template <typename PointType_ikd>
float KD_TREE<PointType_ikd>::calc_dist(PointType_ikd a, PointType_ikd b)
{
    float dist = 0.0f;
...
template <typename PointType_ikd>
float KD_TREE<PointType_ikd>::calc_box_dist(KD_TREE_NODE *node, PointType_ikd point)
{
    if (node == nullptr)
        return INFINITY;
    float min_dist = 0.0;
...
template <typename PointType_ikd>
bool KD_TREE<PointType_ikd>::point_cmp_x(PointType_ikd a, PointType_ikd b) { return a.x < b.x; }
template <typename PointType_ikd>
bool KD_TREE<PointType_ikd>::point_cmp_y(PointType_ikd a, PointType_ikd b) { return a.y < b.y; }
template <typename PointType_ikd>
bool KD_TREE<PointType_ikd>::point_cmp_z(PointType_ikd a, PointType_ikd b) { return a.z < b.z; }
