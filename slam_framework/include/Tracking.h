...
public:
    ...
    /****LiDAR Functions*********/
    Sophus::SE3f GrabImageStereoLidar(const cv::Mat &imRectLeft, const cv::Mat &imRectRight, const double &timestamp,
                                      string filename);
    void SyncWithLidar();
    void BuildLidarFeat4KeyFrame();
...
public:
    ...
    // LiDAR Front-End Variables
protected:
    
    //* sync virables
    int mCurrentLidarId;
    int mLastLidarId;
    PointCloudXYZIN::Ptr currentCloud;

    PointCloudXYZIN::Ptr mCurrInputCloud;
    PointCloudXYZIN::Ptr mLastInputCloud;
    double mCurrCloudTime;

    int mLidarEnabled;

    int mDitchedNum = 0; //* ditched frames
    LIDAR::LidarParam* mpLidarParam;
    bool ParseLiDARParamFile(cv::FileStorage &fSettings);
    //*********************************************/
