            ...
            if(!CheckNewKeyFrames() && !stopRequested())
            {
                if(mpAtlas->KeyFramesInMap()>2)
                {
                    ...
                    if(mbInertial && mpCurrentKeyFrame->GetMap()->isImuInitialized())
                    {
                        ...
                        {
                            OptimizerWithLidar::LocalLVIBA(mpCurrentKeyFrame, &mbAbortBA, mpCurrentKeyFrame->GetMap(),num_FixedKF_BA,num_OptKF_BA,num_MPs_BA,num_edges_BA, bLarge, !mpCurrentKeyFrame->GetMap()->GetIniertialBA2());
                        }
                    ...
                    else
                    {
                        ...
                        {
                            OptimizerWithLidar::LocalLVBundleAdjustment(mpCurrentKeyFrame,&mbAbortBA, mpCurrentKeyFrame->GetMap(),num_FixedKF_BA,num_OptKF_BA,num_MPs_BA,num_edges_BA);
                        }
                        b_doneLBA = true;
                    }
                ...
                {
                    cout << "start IMU Initialization" << endl;
                    if (mbMonocular)
...
void LocalMapping::InitializeIMU(float priorG, float priorA, bool bFIBA)
{
    ...
    if (!mpCurrentKeyFrame->GetMap()->isImuInitialized())
    {
        ...
        dirG = dirG/dirG.norm();
        cout<<"IMU: G direction: " << dirG[0] << " "<< dirG[1] <<" "<< dirG[2] <<endl;
        Eigen::Vector3f gI(0.0f, 0.0f, -1.0f);
        Eigen::Vector3f v = gI.cross(dirG);
        const float nv = v.norm();
        const float cosg = gI.dot(dirG);
        const float ang = acos(cosg);
        cout<<"IMU: Angle between IMU and World: "<< ang/3.1415926*180 << endl;
    ...
    // Before this line we are not changing the map
    {
        ...
        if ((fabs(mScale - 1.f) > 0.00001) || !mbMonocular) {
            ...
            // mpTracker->UpdateFrameIMU(mScale, IMU::Bias (-0.0008, 0.0006, 0.0234,-0.0016, -0.0025, -0.0021), mpCurrentKeyFrame);
    ...
    while(!lpKFtoCheck.empty())
    {
        ...
        if(pKF->bImu)
        {
            ...
            // IMU::Bias (-0.0008, 0.0006, 0.0234,-0.0016, -0.0025, -0.0021)
